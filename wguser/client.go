...
func (c *Client) Devices() ([]*wggtypes.Device, error) {
	devices, err := c.find()
	if err != nil {
		return nil, err
	}

	wgds := make([]*wggtypes.Device, 0, len(devices))
...
func (c *Client) Device(name string) (*wggtypes.Device, error) {
	devices, err := c.find()
	if err != nil {
		return nil, err
	}
...
func (c *Client) ConfigureDevice(name string, cfg wggtypes.Config) error {
	devices, err := c.find()
	if err != nil {
		return err
	}

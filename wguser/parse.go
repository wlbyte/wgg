...
func (c *Client) getDevice(device string) (*wggtypes.Device, error) {
	...
	d.Type = wggtypes.Userspace

	return d, nil
}

// parseDevice parses a Device and its Peers from an io.Reader.
func parseDevice(r io.Reader) (*wggtypes.Device, error) {
	var dp deviceParser
...
// a Device.
func (dp *deviceParser) Device() (*wggtypes.Device, error) {
	if dp.err != nil {
		return nil, dp.err
	}
...
func (dp *deviceParser) Parse(key, value string) {
	...
	case "public_key":
		...
		dp.d.Peers = append(dp.d.Peers, wggtypes.Peer{
			PublicKey: dp.parseKey(value),
		})
		return
	case "bond_name":
		dp.parseBond = true
		dp.d.Bond = &wggtypes.BondConfig{BondName: value}
	}
	...
	// Are we parsing bond fields?
	if dp.parseBond {
		dp.bondParse(key, value)
		// return
	}
...
func (dp *deviceParser) curPeer() *wggtypes.Peer {
...
// bondParse parses a key/value field into the current Bond.
func (dp *deviceParser) bondParse(key, value string) {
	bond := dp.d.Bond
	switch key {
	case "bond_mode":
		bond.BondMode = value
	case "slave_peer":
		bond.SlavePeers = append(bond.SlavePeers, dp.parseKey(value))
	case "tx_bytes":
		bond.TransmitBytes = dp.parseInt64(value)
	case "rx_bytes":
		bond.ReceiveBytes = dp.parseInt64(value)
	case "best_slave_peer":
		bond.BestPeer = dp.parseKey(value)
	case "active_slave_peer":
		bond.ActivePeer = dp.parseKey(value)
	case "allowed_ip":
		cidr := dp.parseCIDR(value)
		if cidr != nil {
			bond.AllowIPs = append(bond.AllowIPs, *cidr)
		}
	}
}

// parseKey parses a Key from a hex string.
func (dp *deviceParser) parseKey(s string) wggtypes.Key {
	if dp.err != nil {
		return wggtypes.Key{}
	}
